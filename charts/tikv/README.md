...
```console
helm template . --name-template=tikv | podman kube play -
```

## Introduction

TiKV is a distributed transactional key-value database that is originally created to complement TiDB.
...
The command deploys TiKV on the Kubernetes cluster in the default configuration. The [Parameters](#parameters) section lists the parameters that can be configured during installation.
